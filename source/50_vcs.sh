...
# From: https://raw.github.com/jakearchibald/git-convenience/master/git-shortcuts.sh
...
g - git
gs - git status
ga - git add ... (. by default)
gaa - Add all changes (including untracted files)
gpu - git push
gp - git pull (via rebase) then push
gup - git pull (via rebase)
gl - git log
...
gb - git branch
gba git branch -a
gco - get checkout ... (master by default)
gcb - create branch (master by default)
gr - git remote
grv - git remote -v
grr - git remote rm
gcl - git clone
gcd - git rev-parse 2>/dev/null && cd "./$(git rev-parse --show-cdup)"
ged - open all changed files (that still actually exist) in the editor
gc - git commit -m
gca "Message" - Commit all new files & changes with message
...
gra - add a github remote by github username
gf - git log with per-commit cmd-clickable GitHub URLs (iTerm) (-n limit)
"
...
# From: https://raw.github.com/git/git/e90020cdb3273af3b0c7915c0aacf16b19bbf994/contrib/completion/git-completion.bash
#!bash
#
