...
Ask the user (with `ask` tool) for the PRD source if it is not already clear.
...
If the PRD is incomplete, ask clarifying questions with `ask` tool before breaking it down.
...
Then ask the user with `ask` tool:
